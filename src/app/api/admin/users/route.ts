...
export async function GET(req: NextRequest) {
  ...
  try {
    const users = role
      ? await sql`SELECT id,name,email,role,verified,created_at FROM users WHERE role=${role} ORDER BY name`
      : await sql`SELECT id,name,email,role,verified,created_at FROM users ORDER BY role,name`
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const verified = role === 'admin' // Admins are auto-verified
    const rows = await sql`
      INSERT INTO users (name,email,password,role,verified)
      VALUES (${name}, ${email.toLowerCase().trim()}, ${hashed}, ${role}, ${verified})
